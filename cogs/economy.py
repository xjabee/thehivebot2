...
class Economy:
    ...
    async def register(self, ctx):
        """Registers an account at the Hive bank"""
        ...
        try:
            ...
            await self.bot.say("{} You already have an account at the"
                               " Hive bank.".format(author.mention))
    ...
    async def _set(self, ctx, user: discord.Member, credits: SetParser):
        ...
        Examples:
            bank set @Hive 26 - Sets 26 credits
            bank set @Hive +2 - Adds 2 credits
            bank set @Hive -6 - Removes 6 credits"""
        author = ctx.message.author
        try:
